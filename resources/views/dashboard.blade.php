    ...
    <div class="p-8 lg:p-12 space-y-12 max-w-[1600px] mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <header
            ...
            <div class="space-y-2">
                ...
                <p class="text-primary/40 text-lg font-medium">Monitoring active capital and credit eligibility for
                    <span class="text-primary font-extrabold">{{ Auth::user()->name }}</span></p>
            </div>

            <div class="flex gap-4">
                <button
                    class="flex items-center gap-2 px-6 py-3 bg-white border border-primary/5 rounded-2xl font-bold text-sm text-primary/60 hover:bg-surface transition-all">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            <div
                ...
                <div
                    class="absolute -right-10 -bottom-10 size-40 bg-accent/5 rounded-full blur-3xl group-hover:bg-accent/10 transition-colors">
                </div>
                ...
                <div class="space-y-1">
                    <h3 class="text-3xl font-display font-black text-primary">
                        {{ $activeLoan ? ucfirst($activeLoan->status) : 'Standby' }}</h3>
                    <p class="text-sm font-bold text-primary/40 uppercase tracking-[0.1em]">
                        {{ $activeLoan ? 'Loan #' . $activeLoan->id : 'No Active Capital' }}</p>
                </div>
                <div class="mt-6 pt-6 border-t border-primary/5 flex items-center gap-2">
                    <span
                        class="flex h-2 w-2 rounded-full {{ $activeLoan && $activeLoan->status == 'active' ? 'bg-accent animate-pulse' : 'bg-primary/20' }}"></span>
            ...
            <div
                ...
                <div class="space-y-1">
                    <h3 class="text-3xl font-display font-black text-primary">{{ Auth::user()->risk_score }}<span
                            class="text-primary/20">/100</span></h3>
                ...
                <div class="mt-6 space-y-2">
                    <div class="w-full h-1.5 bg-surface rounded-full overflow-hidden">
                        <div class="h-full bg-accent transition-all duration-1000"
                            style="width: {{ Auth::user()->risk_score }}%"></div>
                    </div>
            ...
            <div
                ...
                <div class="relative flex justify-between items-start mb-8">
                    <div
                        class="size-14 rounded-2xl bg-red-50 flex items-center justify-center text-red-500 group-hover:bg-red-500 group-hover:text-white transition-all">
                ...
                <div class="space-y-1">
                    <h3 class="text-3xl font-display font-black italic">
                        {{ $nextPayment ? $nextPayment->due_date->format('M d') : 'None' }}</h3>
                    <p class="text-sm font-bold text-primary/40 uppercase tracking-[0.1em]">
                        {{ $nextPayment ? 'Monthly Installment' : 'Next Cycle' }}</p>
                </div>
                <div class="mt-6 flex items-baseline gap-1">
                    <span
                        class="text-xl font-black">{{ $nextPayment ? 'K' . number_format($activeLoan->monthly_payment, 0) : '--' }}</span>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-12">
            ...
            <div class="xl:col-span-2 space-y-8">
                <section
                    class="bg-white rounded-[3.5rem] border border-primary/5 shadow-sm overflow-hidden flex flex-col">
                    <header class="px-12 py-10 border-b border-primary/5 flex items-center justify-between">
                        <div class="space-y-1">
                            ...
                            <p class="text-xs font-bold text-primary/30 uppercase tracking-[0.2em]">Transaction Audit &
                                History</p>
                        </div>
                        <div class="flex items-center gap-4">
                            <div class="flex -space-x-3">
                                @for($i = 1; $i <= 3; $i++)
                                    <div
                                        class="size-10 rounded-full border-4 border-white bg-primary/5 flex items-center justify-center text-[10px] font-black text-primary/30">
                                        S{{$i}}</div>
                                @endfor
                            </div>
                        </div>
                                ...
                                @foreach($loans as $loan)
                                    <tr class="group hover:bg-surface/40 transition-all">
                                        <td class="px-12 py-8">
                                            <div class="flex items-center gap-4">
                                                <div
                                                    class="size-3 bg-primary/10 rounded-full group-hover:scale-150 group-hover:bg-accent transition-all">
                                                </div>
                                                <span
                                                    class="font-display font-black text-xl text-primary">#LN-{{ $loan->id }}</span>
                                            </div>
                                        </td>
                                        <td class="px-12 py-8">
                                            <span
                                                class="px-5 py-2 rounded-[1.5rem] text-[10px] font-black uppercase tracking-[0.2em] shadow-sm
                                        ...
                                        <td class="px-12 py-8 text-sm font-bold text-primary/30 tracking-tighter">
                                            {{ $loan->created_at->format('M d, Y') }}</td>
                                        <td
                                            class="px-12 py-8 text-right font-display font-black text-2xl text-primary tracking-tighter">
                                            K{{ number_format($loan->amount, 0) }}</td>
                                    </tr>
                                @endforeach
                                @empty($loans)
                                    <tr>
                                        <td colspan="4" class="px-12 py-24 text-center opacity-30 select-none">
                                            ...
                                            <p class="font-display font-black text-xl tracking-tight uppercase">No Capital
                                                Records</p>
                                        </td>
                                    </tr>
                                @endempty
            ...
            <div class="xl:col-span-1" id="loan-calculator">
                <div
                    class="sticky top-12 bg-white rounded-[4rem] border border-primary/5 shadow-[0_48px_80px_-16px_rgba(15,23,42,0.1)] p-12 space-y-12">
                    <div class="space-y-4">
                        <div
                            class="flex items-center gap-3 text-accent font-black text-[10px] uppercase tracking-[0.3em]">
                        ...
                        <p class="text-primary/40 font-medium leading-relaxed">Simulate your capital request and receive
                            instant eligibility feedback.</p>
                    </div>
                    ...
                    <form id="calc-form" action="{{ route('loans.store') }}" method="POST" class="space-y-10">
                        ...
                        <div class="space-y-6">
                            <label class="text-[10px] font-black uppercase tracking-[0.3em] text-primary/30">Capital
                                Path</label>
                            <div class="grid grid-cols-1 gap-3">
                                @foreach($products as $product)
                                    <label class="cursor-pointer group">
                                        ...
                                        <div
                                            class="flex items-center justify-between px-6 py-5 rounded-3xl border-2 border-primary/5 bg-surface text-primary/40 group-hover:border-accent/20 peer-checked:bg-primary peer-checked:border-primary peer-checked:text-white transition-all shadow-sm">
                                            <span class="font-bold text-sm tracking-tight">{{ $product->name }}</span>
                                            <span
                                                class="text-xs font-black opacity-40 uppercase tracking-widest">K{{ number_format($product->max_amount / 1000) }}k
                                                Max</span>
                                        </div>
                                    </label>
                                @endforeach
                        ...
                        <div class="space-y-6">
                            <label class="text-[10px] font-black uppercase tracking-[0.3em] text-primary/30">Amount
                                Required</label>
                            <div class="relative group">
                                ...
                                <input type="number" name="amount" id="amount-input" min="1000" max="{{ $maxEligible }}"
                                    step="1" value="1000"
                                    class="block w-full h-20 bg-surface border-2 border-primary/5 focus:border-accent focus:ring-accent rounded-[2rem] pl-12 pr-6 font-display font-black text-4xl text-primary tracking-tighter shadow-sm transition-all">
                            </div>
                            <p
                                class="text-[10px] font-black uppercase tracking-widest text-primary/20 px-2 leading-none">
                                Limit: K{{ number_format($maxEligible) }}</p>
                        </div>

                        <div class="space-y-6">
                            <label class="text-[10px] font-black uppercase tracking-[0.3em] text-primary/30">Horizon
                                (Months)</label>
                            <div class="flex gap-4">
                                @foreach([3, 6, 12] as $months)
                                    <label class="flex-1 cursor-pointer group">
                                        <input type="radio" name="duration_months" value="{{ $months }}" class="hidden peer"
                                            {{ $months == 6 ? 'checked' : '' }}>
                                        <div
                                            class="h-14 flex items-center justify-center rounded-[1.5rem] border-2 border-primary/5 bg-surface font-black text-sm text-primary/30 peer-checked:bg-accent peer-checked:border-accent peer-checked:text-white transition-all">
                                            {{ $months }}m
                                        </div>
                                    </label>
                                @endforeach
                        ...
                        <!-- Result Visual -->
                        <div
                            class="bg-primary p-10 rounded-[3rem] shadow-2xl shadow-primary/40 relative overflow-hidden group">
                            <div
                            ...
                            <div class="relative space-y-6">
                                <div>
                                    <p
                                        class="text-[10px] font-black uppercase tracking-[0.4em] text-white/40 mb-2 leading-none">
                                ...
                                <div class="pt-6 border-t border-white/10 flex justify-between items-center">
                                    <div class="space-y-1">
                                        <p class="text-[9px] font-black uppercase tracking-widest text-white/30">Fixed
                                            Rate</p>
